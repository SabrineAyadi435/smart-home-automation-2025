...
public class CalendarService {
    ...
    /**
     * Creates a personalized and recurring weekly schedule based on your specific
     * requests.
     * 
     * @return A map where the key is the day of the week and the value is the list
     *         of reminders.
     */
    ...
    public List<String> getTodaysReminders() {
        return getRemindersForDate(LocalDate.now());
    }

    /**
     * Get reminders for a specific date based on the DEMO WEEK schedule.
     * 
     * @param date The date to get reminders for.
     * @return A list of strings representing the reminders.
     */
    public List<String> getRemindersForDate(LocalDate date) {
        DayOfWeek day = date.getDayOfWeek();
        List<String> reminders = demoWeekSchedule.getOrDefault(day, Collections.emptyList());

        // CHANGED: Replaced special character with a hyphen
        System.out.println("- Fetching DEMO reminders for: " + userEmail + " (" + day + ")");
        // CHANGED: Replaced special character with a hyphen
        System.out.println("- Found " + reminders.size() + " reminders for " + day);
